...
export const projects: Project[] = [
  {
    ...
    whatDidntWork:
      "The first stack fragmented fast: duplicate institution rows and separate Flask/FastAPI chancing services that drifted out of sync. Collapsing everything into a canonical schema with one chancing service was the fix. The chancing model is also still trained on synthetic cohorts, so it recovers stats-grounded relationships, not validated accuracy against real admits.",
    outcome:
  ...
  },
  {
    ...
    outcome:
      "Dashboard, occupancy, environment tracking, experiments and recommendations all built and running against the seeded 60-day demo. Deploys on free-tier Vercel/Render + Supabase; real cameras are a 2 to 4 week add-on. Next: a public demo link and screenshots in the repo.",
    whatIdDoDifferently:
  ...
  },
  {
    ...
    built: [
      ...
      "Stage 2: canonicalisation. Contaminant aliases, values to PPB, districts to Census 2021, dates to ISO 8601",
      ...
      "Random Forest district-risk model that ranks districts by contamination likelihood",
      "Bayesian supply-chain propagation graph that traces how a contaminant moves through the supply network",
    ...
    metrics: [
      { label: "Pipeline stages", value: "4 (extract to score)" },
    ...
    whatDidntWork:
      "OCR confidence on scanned FSSAI PDFs is the bottleneck. That's why every record carries a confidence score rather than a false promise of certainty. The Random Forest also underfits on the demo dataset; it needs more real records to be meaningful.",
    outcome:
  ...
  },
  {
    ...
    built: [
      ...
      "Degradation suite: resolution (64x64 to 512x512), phase-bits (8 to 1), colour channels, viewing-angle bandwidth, depth planes (1 to 5), speckle (σ 0 to 0.3)",
    ...
    technicalDecisions: [
      "Angular-Spectrum Method: exact diffraction propagation, valid when pixel pitch is close to wavelength",
    ...
    whatDidntWork:
      "1-bit phase quantisation is a true cliff (SSIM collapses to ~0.003) while 2-bit survives. That's the kind of non-obvious threshold the framework exists to surface.",
    outcome:
  ...
  },
  {
    ...
    built: [
      "Webcam to MediaPipe to One-Euro filter to posture classifier to per-hand control-bus pipeline",
    ...
    whatDidntWork: "",
    outcome:
      "Core vision to gesture to audio pipeline is wired; time-stretch, beat-repeat/reverse buffers, Spotify OAuth's redirect handling, and onboarding UI are still open, tracked as TODOs in the repo.",
    whatIdDoDifferently: "",
  ...
  },
  {
    ...
    built: [
      ...
      "Timed research to timed spoken-recall flow with audio recording",
  ...
  },
  {
    ...
    approach: "Lead with the mechanism (SO₂ to fertiliser) and the economics (zero capex, managed service) before anything else.",
  ...
  },
  {
    ...
    longDescription:
      "Azolla ferns absorb excess nitrogen and phosphorus at the surface; Daphnia consume algae at the base. Together they attack eutrophication from both ends of the water column with no chemicals, no machinery, 60 days. I built the protocol, modelled the finances, and designed a carbon-neutral deployment boat.",
